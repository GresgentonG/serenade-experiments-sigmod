...
DATA_PATH = '/Users/crescentonc/code/comp5331/datasets_dropbox/rsc15/raw/'
...
def load_data( file ) : 
    ...
    data = data[data.size // 10 : data.size // 7]
    #specify header names
    data.columns = ['SessionId', 'TimeStr', 'ItemId']
    
    #convert time string to timestamp and remove the original column
    def parse(x):
        return datetime.strptime(x, '%Y-%m-%dT%H:%M:%S.%fZ').timestamp()
    data['Time'] = data.TimeStr.apply(parse) #This is not UTC. It does not really matter.
    del(data['TimeStr'])
...
def split_data_slice( data, output_file, slice_id, days_offset, days_train, days_test ) :
    ...
    train_hyperparam = train[:train.size // 50]
    ...
    test_hyperparam = test[:test.size // 50]
