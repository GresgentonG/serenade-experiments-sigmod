...
impl RocksDBSessionStore {
    pub fn new(database_file: &str, ttl: Duration) -> Self {
        ...
        let rocks_db = DB::open_with_ttl(&options, database_file, ttl).unwrap();

        Self {
            rocks_db,
            max_session_idle_duration_in_seconds: 60 * 20,
        }
    }
    ...
    pub fn update_session_items(&self, evolving_session_id: &u128, session_items: &[u64]) {
        let serialized_session_id = bincode::serialize(evolving_session_id).unwrap();
        let now = self.get_seconds_since_epoch();
        let payload = DBValue {
